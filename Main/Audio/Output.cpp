...
static uint32_t bufferLength = 256;
static uint32_t numBuffers = 2;
static REFERENCE_TIME bufferDuration = (REFERENCE_TIME)(0.04 * REFTIMES_PER_SEC);
...
bool Output::Begin(float*& buffer, uint32_t& numSamples)
{
...
void Output::End(uint32_t numSamples)
{
	if(numSamples > 0)
	{
