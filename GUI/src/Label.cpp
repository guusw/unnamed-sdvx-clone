...
void Label::Render(GUIRenderData data)
{
	...
	{
		m_textTransform = Transform2D(m_cachedArea.pos) * m_cachedTransform;
	}
	...
	if(data.debug)
	{
		Rect debug = Rect(textObject->textBounds.pos + m_cachedArea.pos, textObject->textBounds.size);
		data.guiRenderer->RenderWireBox(debug.ToTransform(), Color::FromHSV(99.0f, 0.6f, 0.8f));
		debug = Rect(m_cachedArea.pos, textObject->size);
		data.guiRenderer->RenderWireBox(debug.ToTransform(), Color::FromHSV(99.0f, 0.6f, 0.8f));
	}
}
